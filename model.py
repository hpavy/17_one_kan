...
class KAN(torch.nn.Module):
    def __init__(
        ...
        self.grid_size = hyper_param['grid_size']
        self.spline_order = spline_order

        self.layers = torch.nn.ModuleList()
        for in_features, out_features in zip(hyper_param['hiden_layer'], hyper_param['hiden_layer'][1:]):
            self.layers.append(
                KANLinear(
                    in_features,
                    out_features,
                    grid_size=hyper_param['grid_size'],
                    ...
                    grid_eps=grid_eps,
                    grid_range=hyper_param['grid_range'],
                )
            )
